        ...
        where TUser : Entities.BaseUser, new()
    {
        ...
        public async Task<AuthResult<Token>> SignUp(SignUpDTO signUpDto)
        {
            if (signUpDto == null ||
                ...
                signUpDto.GenderId == 0 ||
                signUpDto.CityId == 0 ||
                signUpDto.StateId == 0
            )
                return AuthResult<Token>.UnvalidatedResult;

            var agreementDateTime = DateTime.UtcNow;

            var newUser = new TUser { UserName = signUpDto.Email, Email = signUpDto.Email,  
                                      FirstName = signUpDto.FullName, Age = signUpDto.Age, 
                                      GenderId = signUpDto.GenderId, StateId = signUpDto.StateId,
                                      CityId = signUpDto.CityId, AcceptCovidAgreement = signUpDto.AcceptCovidAgreement,
                                      AcceptUserAgreement = signUpDto.AcceptUserAgreement, AcceptCovidAgreementDate = agreementDateTime,
                                      AcceptUserAgreementDate = agreementDateTime
            };
